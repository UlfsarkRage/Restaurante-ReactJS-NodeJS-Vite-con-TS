...
// 1. Definimos la forma de los datos que el cliente envía
interface PeticionPlato {
  tipoCliente: string;
  plato: string;
}

// 2. Definimos la forma de la respuesta que el backend enviará
...
export function procesarPedido(req: Request<{}, {}, PeticionPlato>, res: Response) {
  ...
  console.log(
    `MESERO(Controlador): El chef me devolvió el plato. Devuelvo el resultado al cliente.`
  );
  res.json(resultado);
}